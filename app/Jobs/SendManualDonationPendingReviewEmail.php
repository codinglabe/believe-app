...
use App\Mail\ManualDonationPendingReviewMail;
...
class SendManualDonationPendingReviewEmail implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            Mail::to($email)->send(new ManualDonationPendingReviewMail(
                $donation,
                $donor,
                $recipientLabel,
            ));
        } catch (\Throwable $e) {
            Log::error('Failed to send manual donation pending review email', [
