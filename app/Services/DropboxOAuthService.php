...
class DropboxOAuthService
{
    ...
    /**
     * Get a valid Dropbox access token for a user (supporter) for VDO.Ninja Cloud Sync.
     * Refreshes the token if necessary. Returns null if user has no Dropbox linked or refresh fails.
     */
    public function getAccessTokenForUser(User $user): ?string
    {
        ...
        $cachedAccess = $user->dropbox_access_token;
        $expiresAt = $user->dropbox_token_expires_at;
        if ($cachedAccess && $expiresAt && $expiresAt->isAfter(now()->addMinutes(5))) {
            try {
                return Crypt::decryptString($cachedAccess);
            } catch (\Throwable $e) {
        ...
        try {
            $refreshToken = Crypt::decryptString($user->dropbox_refresh_token);
        } catch (\Throwable $e) {
            Log::warning('Dropbox refresh token decrypt failed (user)', ['user_id' => $user->id]);
            return null;
        }
        ...
        if (! $response->successful()) {
            Log::warning('Dropbox token refresh failed (user)', [
                'user_id' => $user->id,
                'body' => $response->body(),
            ]);
            return null;
