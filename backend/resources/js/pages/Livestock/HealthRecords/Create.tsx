...
export default function CreateHealthRecord({ animal }: CreateProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault()
        post(route('health.store', animal.id), {
            onSuccess: () => {
                showSuccessToast('Health record added successfully.')
            },
            onError: () => {
                showErrorToast('Failed to add health record.')
            }
        })
    }
    ...
    return (
        <LivestockDashboardLayout breadcrumbs={breadcrumbs}>
            <Head title={`Add Health Record - ${animal.breed}`} />
            
            <div className="max-w-3xl mx-auto">
                <Card>
                    <CardHeader>
                        <CardTitle>Add Health Record for {animal.breed}</CardTitle>
                    </CardHeader>
                    <CardContent>
