...
interface PageProps {
  ...
  alliances: PaginatedAlliances
  searchQuery: string
  filters: Filters
  /** Active primary action categories (alliance hub + org registration) */
...
export default function FindCareAlliancesPage() {
  const {
    seo,
  ...
  const toggleCardInterests = (allianceId: number) => {
  ...
  const applyLocation = useCallback(
    (locationValue: string) => {
      ...
      router.get(route("find-care-alliances.index"), buildParams(searchQuery, nextFilters), {
        preserveState: false,
      })
    },
    [filters, searchQuery]
  ...
  React.useEffect(() => {
    ...
    const timer = setTimeout(() => {
      router.get(route("find-care-alliances.index"), buildParams(searchQueryRef.current, filters), {
        preserveState: false,
      })
    }, 500)
    return () => clearTimeout(timer)
  ...
  const clearSameCauses = useCallback(() => {
    ...
    setFilters(next)
    router.get(route("find-care-alliances.index"), buildParams(searchQuery, next), {
      preserveState: false,
    })
  }, [filters, searchQuery])
  ...
  }, [initialAlliances, initialQuery, initialFilters])

  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault()
    // Clear any pending debounce and search immediately
    router.get(route("find-care-alliances.index"), buildParams(searchQuery, filters), {
      preserveState: false,
    })
  }
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
            ...
            <div className="lg:col-span-9 space-y-6">
              <form onSubmit={handleSearch} className="relative group">
                ...
                <Input
                  type="text"
                  placeholder="Search alliances by name, location, or keywords..."
                  value={searchQuery}
              ...
              {selectedCauseNames.length > 0 && (
                <p className="text-sm text-gray-500 dark:text-gray-400">
                  ...
                  {alliances.total > 0 && ` • ${alliances.total} alliance${alliances.total !== 1 ? "s" : ""}`}
                </p>
              )}

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {alliances?.data?.map((row) => (
                  <Card
                    key={row.id}
                  ...
                  >
                    <CardContent className="p-5">
                      <div className="flex items-start gap-3">
                        <Link
                          href={route("alliances.show", row.slug)}
                          className="flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-700 text-white shadow-md"
                          aria-hidden
                        >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {row.description_preview && (
                            <p className="mt-2 text-sm text-gray-600 dark:text-gray-400 line-clamp-3">{row.description_preview}</p>
                          )}
                          <div className="mt-3 flex flex-wrap items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
                            <span className="inline-flex items-center gap-1.5">
                              <Users className="w-4 h-4 text-blue-500" />
                              <span>
                                {row.active_members_count} member{row.active_members_count !== 1 ? "s" : ""}
                              </span>
                            </span>
                            <span className="inline-flex items-center gap-1.5">
                              <Megaphone className="w-4 h-4 text-violet-500" />
                              <span>
                                {row.active_campaigns_count} active campaign{row.active_campaigns_count !== 1 ? "s" : ""}
                              </span>
                            </span>
                          </div>
                          {row.interests && row.interests.length > 0 && (() => {
                            const showAll = expandedInterestCards.has(row.id)
                            const visible = showAll ? row.interests : row.interests.slice(0, INTERESTS_VISIBLE_COUNT)
                            const remaining = row.interests.length - INTERESTS_VISIBLE_COUNT
                            ...
                            return (
                              <div className="flex flex-wrap items-center gap-1.5 mt-2">
                                ...
                                {hasMore && (
                                  <button
                                    type="button"
                                    onClick={() => toggleCardInterests(row.id)}
                          ...
                          <div className="flex items-center justify-end mt-4 pt-4 border-t border-gray-100 dark:border-gray-800">
                            <Button
                              size="sm"
                              asChild
                              className="rounded-xl font-semibold px-5 bg-gradient-to-r from-blue-600 to-blue-700 text-white hover:from-blue-700 hover:to-blue-800 shadow-md hover:shadow-lg"
                            >
                              <Link href={route("alliances.show", row.slug)}>View alliance</Link>
                            </Button>
                          </div>
                        </div>
                      </div>
              ...
              {alliances?.next_page_url && (
                <div className="flex justify-center pt-6">
                  <Button
                    variant="outline"
                    onClick={() => router.get(alliances.next_page_url!)}
