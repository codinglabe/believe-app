...
interface Transaction {
  ...
  type: "deposit" | "withdrawal" | "purchase" | "refund" | "transfer" | "donation" | "commission" // Added commission
  ...
  processed_at: string | null // Can be null for pending transactions
  created_at: string
  updated_at: string
}
...
function TransactionDetailDialog({
  ...
  const getAmountColorClass = (type: Transaction["type"]) => {
    switch (type) {
      case "deposit":
      case "refund":
      case "commission":
      case "donation": // Assuming donation adds to balance for the user receiving it
        return "text-green-600 dark:text-green-400"
      case "withdrawal":
      case "purchase":
      case "transfer": // Assuming transfer is a debit from this user's perspective
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-xl md:max-w-3xl bg-white dark:bg-gray-900 text-gray-900 dark:text-white border border-gray-200 dark:border-gray-700 rounded-xl shadow-2xl p-6">
        ...
        <Tabs defaultValue="overview" className="w-full">
          ...
          <TabsContent value="overview" className="mt-4 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Status Card */}
              <Card className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-4 rounded-lg">
              ...
              {/* Amount Card */}
              <Card className="bg-green-50 dark:bg-green-700/30 border border-green-200 dark:border-green-600 p-4 rounded-lg">
                <CardContent className="p-0">
                  <p className="text-sm text-green-700 dark:text-green-300 mb-2">Amount</p>
                  <div className="flex items-center gap-2">
                    <DollarSign className="h-6 w-6 text-green-600 dark:text-green-400" />
                    <p className={`text-xl font-bold ${getAmountColorClass(transaction.type)}`}>
                      {`${transaction.currency} ${Number(transaction.amount).toLocaleString(undefined, {
              ...
              {/* Fee Card */}
              <Card className="bg-red-50 dark:bg-red-700/30 border border-red-200 dark:border-red-600 p-4 rounded-lg">
                <CardContent className="p-0">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-xl font-bold text-red-800 dark:text-red-400">
                      {`${transaction.currency} ${Number(transaction.fee).toLocaleString(undefined, {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}`}
                    </p>
            ...
            <Card className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-4 rounded-lg">
              <CardContent className="p-0 space-y-3">
                ...
                <div className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
                  ...
                  <span>
                    Payment Method:{" "}
                    <span className="font-semibold capitalize">{transaction.payment_method || "N/A"}</span>
                  </span>
                </div>
                <div className="flex items-center gap-2 text-gray-700 dark:text-gray-300">
          ...
          <TabsContent value="meta" className="mt-4">
            <Card className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-4 rounded-lg">
              <CardContent className="p-0">{renderMetaData(transaction.meta || {})}</CardContent>
            </Card>
          </TabsContent>
          <TabsContent value="audit" className="mt-4">
            <Card className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 p-4 rounded-lg">
              <CardContent className="p-0 space-y-3">
                ...
                <div className="flex flex-col">
                  <span className="text-sm font-medium text-gray-500 dark:text-gray-400">Processed At:</span>
                  <span className="text-base font-semibold text-gray-900 dark:text-white">
                    {transaction.processed_at ? new Date(transaction.processed_at).toLocaleString() : "N/A (Pending)"}
                  </span>
                </div>
              </CardContent>
            </Card>
  ...
  return (
    <ProfileLayout title="Transaction History" description="Track your product orders and deliveries">
      ...
      <motion.div
        ...
        className="bg-white dark:bg-gray-900 rounded-lg overflow-hidden border border-gray-200 dark:border-gray-800 mb-6"
      >
      ...
      <motion.div
        ...
        className="rounded-lg overflow-hidden  border-gray-200 dark:border-gray-800"
      >
        {/* Main content area for transaction cards */}
        <CardContent className="!p-0 grid grid-cols-1 gap-4 p-4">
            ...
            {transactions.data.length > 0 ? (
              transactions.data.map((transaction) => (
                <motion.div
                  ...
                  className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 relative overflow-hidden cursor-pointer hover:shadow-md transition-shadow duration-200"
                  onClick={() => handleViewDetails(transaction)}
                >
                  {/* Top section: Type, ID, Status, Buttons */}
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex flex-col">
                      <h3 className="text-xl font-bold text-gray-700 dark:text-gray-50 capitalize flex items-center gap-2">
                        {getTypeIcon(transaction.type)}
                        {transaction.type.replace(/_/g, " ")}
                      </h3>{" "}
                      <div className="flex items-center gap-2 text-sm mt-1">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded-full text-xs font-semibold capitalize ${getStatusBadgeClass(
                            transaction.status,
                          )}`}
                        >
                          {transaction.status}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-2 gap-x-6 text-gray-300">
                    <div className="flex justify-between sm:block">
                      ...
                      <span className={`font-semibold sm:ml-2 ${getAmountColorClass(transaction.type)}`}>
                        {`${transaction.currency} ${Number(transaction.amount).toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                          maximumFractionDigits: 2,
                        })}`}
                    ...
                    <div className="flex justify-between sm:block">
                      ...
                      <span className="font-semibold text-gray-900 dark:text-white sm:ml-2 capitalize">
                    ...
                    {transaction.fee > 0 && (
                      <div className="flex justify-between sm:block">
                        <span className="text-gray-700 dark:text-gray-200">Fee:</span>
                        <span className="font-semibold text-red-400 sm:ml-2">
                          {`${transaction.currency} ${Number(transaction.fee).toLocaleString(undefined, {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2,
                          })}`}
