...
const helperPrompts = {
  helps_who: "Who are you serving? Describe the beneficiaries. Use plain language.",
  fund_usage: "What will donations fund in the next 3–6 months? Be specific.",
        ...
        <Card className="px-0">
              ...
              <div>
                ...
                {campaign.rejection_reason && (
                  <p className="text-sm text-amber-600 dark:text-amber-400 mt-2">
                    Reviewer note: {campaign.rejection_reason}
                  </p>
                )}
              </div>
            </div>
          ...
          <CardContent className="px-4 md:px-6">
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                <Label>Cover image {!campaign.cover_image_path ? "*" : ""}</Label>
                {campaign.cover_image_path ? (
                  <p className="text-xs text-muted-foreground">Current image in use. Upload a new file to replace.</p>
                ) : (
                  <p className="text-xs text-muted-foreground">Required to publish. JPEG, PNG, GIF or WebP. Max 5 MB.</p>
                )}
                <Input
                  type="file"
              ...
              <div className="flex flex-wrap gap-3 pt-4">
                ...
                <Button
                  ...
                  value="in_review"
                  variant="default"
                  disabled={isSubmitting || (!campaign.cover_image_path && !formData.cover_image)}
                  title={!campaign.cover_image_path && !formData.cover_image ? "Add a cover image to publish" : undefined}
                >
                  Publish (go live)
                </Button>
                <Link href={route("fundme.campaigns.index")}>
