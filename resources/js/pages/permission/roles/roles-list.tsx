...
export default function RolesList({ roles: initialRolesData, allPermissions, onNavigate }: RolesListProps) {
  const [roles, setRoles] = useState(initialRolesData.data)
  console.log(roles)
  const [searchTerm, setSearchTerm] = useState("")
  const [currentPage, setCurrentPage] = useState(initialRolesData.current_page)
  const itemsPerPage = initialRolesData.per_page
  const totalPages = initialRolesData.last_page
  const totalItems = initialRolesData.total
                  ...
                  <div className="flex items-center gap-1">
                    <Link href={route('roles.edit', role.id)} className=" cursor-pointer">
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={() => onNavigate("edit-role", role.id)}
