...
interface EmailPackagesEditProps {
    package: EmailPackage
}

export default function AdminEmailPackagesEdit({ package: pkg }: EmailPackagesEditProps) {
    const { data, setData, put, processing, errors } = useForm({
        name: pkg.name,
        description: pkg.description || '',
        emails_count: pkg.emails_count,
        price: pkg.price,
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault()
        put(`/admin/email-packages/${pkg.id}`)
    }

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Edit Email Package" />
            
            <div className="space-y-6 p-4 sm:p-6">
                {/* Header */}
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Edit Email Package</h1>
                        <p className="text-gray-600 dark:text-gray-400 mt-1.5">
                            Update the email package details
                        </p>
                    </div>
                ...
                <form onSubmit={handleSubmit}>
                    <Card>
                        <CardHeader>
                            ...
                            <CardDescription>
                                Update the details for the email package
                            </CardDescription>
                        </CardHeader>
