...
import { Search, Filter, Eye, ExternalLink, FileCheck, DollarSign } from "lucide-react"
...
const breadcrumbs: BreadcrumbItem[] = [
  ...
  { title: "State Sales Tax Rates", href: "#" },
]
...
export default function AdminStateSalesTaxIndex({ states, filters }: PageProps) {
  ...
  const handleFilterChange = (filterName: string, value: string) => {
    ...
    router.get(route("admin.state-sales-tax.index"), params, { preserveState: true })
  }

  const handleSearch = (value: string) => {
    setSearchQuery(value)
    const params: Record<string, string> = { ...filters, search: value }
    router.get(route("admin.state-sales-tax.index"), params, { preserveState: true, replace: true })
  }

  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Head title="State Sales Tax Rates Management" />

      <div className="space-y-6 p-4 sm:p-6">
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-white">State Sales Tax Rates</h1>
            <p className="text-gray-600 dark:text-gray-400 mt-1.5">
              View and manage sales tax rates for all 50 states. Total: {states.total} states
            </p>
          </div>
        </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-left p-3 font-semibold">Services/Goods</th>
                ...
                <tbody>
                  {states.data.map((state) => (
                    <tr key={state.id} className="border-b hover:bg-gray-50 dark:hover:bg-gray-800">
                      ...
                      <td className="p-3">{getServicesVsGoodsBadge(state.services_vs_goods)}</td>
                      <td className="p-3">
                      ...
                      <td className="p-3">
                        <Link href={route("admin.state-sales-tax.show", state.id)}>
