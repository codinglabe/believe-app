...
interface PromotionalBannersEditProps {
    banner: PromotionalBanner
}

export default function AdminPromotionalBannersEdit({ banner }: PromotionalBannersEditProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        // Add all form fields except image_url (which we handle conditionally)
        Object.keys(data).forEach(key => {
            if (key === 'image_url') return // Skip, handle separately
        ...
        // Handle image_url based on situation:
        // - If new image uploaded: don't send image_url (backend will upload new file)
        // - If type is text: send empty string (backend will delete old image)
        // - If type is image and no new image: don't send image_url (backend will preserve existing)
        if (data.type === 'text') {
            formData.append('image_url', '')
        }
        // Otherwise, don't append image_url - backend will handle appropriately
        
        // Add method spoofing for PUT request
        formData.append('_method', 'PUT')
        
        // Use router.post with FormData for proper file upload handling
        router.post(`/admin/promotional-banners/${banner.id}`, formData, {
            forceFormData: true,
            preserveScroll: true,
        })
    }
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Edit Promotional Banner" />
            
            <div className="space-y-6 p-4 sm:p-6">
                {/* Header */}
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Edit Promotional Banner</h1>
