...
interface PageProps {
  ...
  directBridgeSubmission: boolean
}
...
export default function AdminKybVerificationIndex({ submissions, stats, filters, directBridgeSubmission: initialDirectBridge }: PageProps) {
  const [searchTerm, setSearchTerm] = useState(filters.search || "")
  const [statusFilter, setStatusFilter] = useState(filters.status || "all")
  const [activeTab, setActiveTab] = useState("pending")
  const [directBridgeSubmission, setDirectBridgeSubmission] = useState(initialDirectBridge)
  const [isUpdatingSetting, setIsUpdatingSetting] = useState(false)

  // Sync state with props when page reloads
  useEffect(() => {
    setDirectBridgeSubmission(initialDirectBridge)
  }, [initialDirectBridge])
  ...
  const handleTabChange = (value: string) => {
    setActiveTab(value)
    router.get(
      route("admin.kyb-verification.index"),
      {
  ...
  const handleToggleDirectBridge = async (enabled: boolean) => {
    setIsUpdatingSetting(true)
    try {
      const response = await fetch(route('admin.kyb-verification.settings.direct-bridge-submission'), {
        method: 'POST',
        headers: {
      ...
      if (response.ok && data.success) {
        const serverValue = data.enabled !== undefined ? data.enabled : enabled
        setDirectBridgeSubmission(serverValue)
        showSuccessToast(
          serverValue 
            ? 'Direct Bridge submission enabled. New KYB submissions will be sent to Bridge immediately.'
            : 'Approval mode enabled. New KYB submissions will require admin approval before sending to Bridge.'
        )
        router.reload({ 
          only: ['directBridgeSubmission'],
          preserveState: false 
        })
      } else {
      ...
      showErrorToast(error instanceof Error ? error.message : 'Failed to update setting')
      setDirectBridgeSubmission(!enabled)
    } finally {
  ...
  const filteredSubmissions = useMemo(() => {
    return submissions.data.filter((submission) => {
      const matchesSearch =
        ...
        submission.business_email?.toLowerCase().includes(searchTerm.toLowerCase()) ||
        submission.ein?.includes(searchTerm) ||
  ...
  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Head title="KYB Verification Center" />
          ...
          <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
            <div>
              <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text text-transparent">
                KYB Verification Center
              </h1>
              <p className="text-muted-foreground mt-1">Manage and monitor business identity verification</p>
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
            <Card className="border-border/50 bg-card/50 backdrop-blur">
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground mt-1">All KYB submissions</p>
              </CardContent>
            </Card>
              ...
              <Card className="border-border/50 bg-card/50 backdrop-blur">
                <CardHeader className="pb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle>KYB Verifications</CardTitle>
                  </div>
                  <CardDescription>Review and manage business identity verification submissions</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="mb-4 flex flex-col gap-4 sm:flex-row">
                    <div className="relative flex-1">
                      ...
                      <Input
                        placeholder="Search by business name, email, EIN, or customer ID..."
                                ...
                                <tr key={submission.id} className="hover:bg-muted/20 transition-colors">
                                  <td className="px-4 py-4">
                                    <div className="flex items-center gap-3">
                                      ...
                                      <div>
                                        <div className="font-medium text-foreground">{submission.business_name || "N/A"}</div>
                                  ...
                                  <td className="px-4 py-4">
                                    <div className="flex justify-end gap-1">
                                      <Link href={route("admin.kyb-verification.show", submission.id)}>
