...
interface PageProps {
  kiosk: KioskCategoryData
}

export default function KioskCategoryEdit({ kiosk }: PageProps) {
  const { data, setData, put, processing, errors } = useForm({
    title: kiosk.title,
    keywords: kiosk.keywords ?? "",
    redirect_url: kiosk.redirect_url ?? "",
    sort_order: kiosk.sort_order ?? 0,
    is_active: kiosk.is_active,
  })

  const submit = (e: React.FormEvent) => {
    e.preventDefault()
    put(route("admin.kiosk.update-category", kiosk.id))
  }
  ...
  const breadcrumbs = [
    ...
    { title: "Edit", href: route("admin.kiosk.categories.edit", kiosk.id) },
  ]

  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Head title={`Edit category — ${kiosk.title}`} />
      <div className="space-y-6 p-4 sm:p-6 w-full max-w-none min-h-screen">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">
              Slug: <span className="font-medium text-foreground">{kiosk.slug}</span>
            </p>
          </div>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Update the category shown on the public Kiosk page.</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={submit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input id="slug" value={kiosk.slug} disabled />
              </div>
