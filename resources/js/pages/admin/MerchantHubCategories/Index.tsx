...
interface MerchantHubCategoriesIndexProps {
...
export default function AdminMerchantHubCategoriesIndex({ categories }: MerchantHubCategoriesIndexProps) {
    const [deletingId, setDeletingId] = useState<number | null>(null)
    const [deleteDialogOpen, setDeleteDialogOpen] = useState(false)
    const [categoryToDelete, setCategoryToDelete] = useState<MerchantHubCategory | null>(null)

    const handleDelete = (category: MerchantHubCategory) => {
    ...
    const handleDeleteConfirm = () => {
        ...
        router.delete(`/admin/merchant-hub-categories/${categoryToDelete.id}`, {
            onFinish: () => {
                setDeletingId(null)
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Merchant Hub Categories Management" />

            <div className="space-y-6 p-4 sm:p-6">
                {/* Header Section */}
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Merchant Hub Categories</h1>
                        <p className="text-gray-600 dark:text-gray-400 mt-1.5">
                            Manage categories for merchant hub offers. Total: {categories.length} categories
                        </p>
                    </div>
                    <div className="flex gap-2">
                        <Link href="/admin/merchant-hub-categories/create">
                ...
                {categories.length === 0 ? (
                    <div className="p-16 text-center">
                        <div className="flex flex-col items-center gap-4">
                            ...
                            <div>
                                <p className="text-xl font-semibold text-gray-900 dark:text-white mb-1.5">No categories created yet</p>
                                ...
                                <Card className={`group h-full flex flex-col hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border-gray-200 dark:border-gray-800 overflow-hidden`}>
                                    ...
                                    <CardContent className="px-6 pb-6 space-y-3 flex-1 flex flex-col">
                                        ...
                                        <div className="flex items-center justify-end gap-2 pt-4 border-t border-gray-200 dark:border-gray-700 mt-auto">
                                            <Link href={`/admin/merchant-hub-categories/${category.id}/edit`}>
                                                <Button
                                                    variant="ghost"
                                                    size="sm"
                    ...
                    <DialogContent>
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                ...
                                This action cannot be undone. Categories with existing offers cannot be deleted.
                            </DialogDescription>
                        </DialogHeader>
                        <DialogFooter>
                            <Button
