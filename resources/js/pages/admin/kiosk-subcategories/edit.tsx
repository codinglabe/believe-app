"use client"

import AppLayout from "@/layouts/app-layout"
import { Head, Link, useForm } from "@inertiajs/react"
import { Button } from "@/components/ui/button"
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card"
import { Input } from "@/components/ui/input"
import { Label } from "@/components/ui/label"
import { Layers, ArrowLeft } from "lucide-react"

interface SubcategoryData {
  id: number
  category_slug: string
  name: string
  sort_order: number
}

interface PageProps {
  subcategory: SubcategoryData
  categories: { value: string; label: string }[]
}

export default function KioskSubcategoriesEdit({ subcategory, categories }: PageProps) {
  const { data, setData, put, processing, errors } = useForm({
    category_slug: subcategory.category_slug,
    name: subcategory.name,
    sort_order: subcategory.sort_order,
  })

  const submit = (e: React.FormEvent) => {
    e.preventDefault()
    put(route("admin.kiosk.subcategories.update", subcategory.id))
  }

  const listHref = `${route("admin.kiosk.subcategories.index")}?category=${encodeURIComponent(subcategory.category_slug)}`

  const breadcrumbs = [
    { title: "Dashboard", href: "/dashboard" },
    { title: "Kiosk Management", href: route("admin.kiosk.index") },
    { title: "Subcategories", href: listHref },
    { title: "Edit", href: route("admin.kiosk.subcategories.edit", subcategory.id) },
  ]

  return (
    <AppLayout breadcrumbs={breadcrumbs}>
      <Head title={`Edit: ${subcategory.name}`} />
      <div className="space-y-6 p-4 sm:p-6 w-full max-w-none min-h-screen">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" asChild>
            <Link href={listHref}><ArrowLeft className="h-4 w-4" /></Link>
          </Button>
          <div>
            <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
              <Layers className="h-7 w-7 text-primary" />
              Edit Subcategory
            </h1>
            <p className="text-muted-foreground text-sm mt-1">{subcategory.name}</p>
          </div>
        </div>

        <Card>
          <CardHeader>
            <CardTitle>Subcategory details</CardTitle>
            <CardDescription>Changes will affect the options shown when creating or editing Kiosk items.</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={submit} className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="category_slug">Category *</Label>
                <select
                  id="category_slug"
                  value={data.category_slug}
                  onChange={(e) => setData("category_slug", e.target.value)}
                  className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm h-9"
                >
                  {categories.map((c) => (
                    <option key={c.value} value={c.value}>{c.label}</option>
                  ))}
                </select>
                {errors.category_slug && <p className="text-sm text-destructive">{errors.category_slug}</p>}
              </div>

              <div className="space-y-2">
                <Label htmlFor="name">Name *</Label>
                <Input
                  id="name"
                  value={data.name}
                  onChange={(e) => setData("name", e.target.value)}
                  placeholder="e.g. Medicaid (Healthy Louisiana)"
                  className={errors.name ? "border-destructive" : ""}
                />
                {errors.name && <p className="text-sm text-destructive">{errors.name}</p>}
              </div>

              <div className="space-y-2">
                <Label htmlFor="sort_order">Sort order</Label>
                <Input
                  id="sort_order"
                  type="number"
                  min={0}
                  value={data.sort_order}
                  onChange={(e) => setData("sort_order", parseInt(e.target.value, 10) || 0)}
                />
                {errors.sort_order && <p className="text-sm text-destructive">{errors.sort_order}</p>}
              </div>

              <div className="flex justify-end gap-3 pt-4">
                <Button type="button" variant="outline" asChild>
                  <Link href={listHref}>Cancel</Link>
                </Button>
                <Button type="submit" disabled={processing}>
                  {processing ? "Saving..." : "Save changes"}
                </Button>
              </div>
            </form>
          </CardContent>
        </Card>
      </div>
    </AppLayout>
  )
}
