// components/buy-share-modal.tsx
"use client"
...
interface BuyShareModalProps {
  ...
  onClose: () => void
  nodebox: NodeBox
  onPurchaseSuccess: (nodeboxId: string, amount: number) => void
}

const suggestedAmounts = [10, 25, 50, 100, 250]

export default function BuyShareModal({ isOpen, onClose, nodebox, onPurchaseSuccess }: BuyShareModalProps) {
  ...
  const handlePurchase = async () => {
    ...
    setIsSuccess(true)
    onPurchaseSuccess(nodebox.id, amountToPurchase)

    // Reset form after a delay or when modal is closed
    // setTimeout(() => {
    //   setIsSuccess(false);
    //   onClose();
    //   setSelectedAmount(null);
    ...
    // }, 5000); // Keep success screen for 5 seconds
  }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleCloseModal}>
      <DialogContent className="bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 max-w-7xl max-h-[90vh] overflow-y-auto">
        {isSuccess ? (
