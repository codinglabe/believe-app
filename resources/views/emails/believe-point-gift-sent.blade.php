...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    <div style="background: linear-gradient(135deg, #7c3aed, #2563eb); padding: 28px; border-radius: 8px 8px 0 0; text-align: center;">
        <h1 style="color: #fff; margin: 0; font-size: 22px;">Gift sent successfully</h1>
    </div>
    <div style="background-color: #f8f9fa; padding: 28px; border-radius: 0 0 8px 8px;">
        <p style="font-size: 16px;">Hi {{ $senderName }},</p>
        <p style="font-size: 16px;">
            You sent <strong>{{ $amountLabel }} BP</strong> to <strong>{{ $recipientName }}</strong>
            @if ($occasion) for <strong>{{ $occasion }}</strong>@endif.
        </p>
        @if ($messageText)
            <p style="font-size: 14px; color: #666;">Your note: “{{ $messageText }}”</p>
        @endif
