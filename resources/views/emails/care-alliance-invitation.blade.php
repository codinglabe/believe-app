...
<head>
    ...
    <title>Unity Impact Alliance invitation — {{ $allianceName }}</title>
</head>
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
    <div style="background: linear-gradient(to bottom, #4f46e5, #7c3aed); padding: 24px; border-radius: 8px 8px 0 0; text-align: center;">
        <h1 style="color: #fff; margin: 0; font-size: 22px;">Unity Impact Alliance invitation</h1>
    </div>
    ...
    <div style="background-color: #f8f9fa; padding: 28px; border-radius: 0 0 8px 8px;">
        ...
        <p style="font-size: 16px; margin-bottom: 16px;">
            <strong>{{ $inviterName }}</strong> has invited <strong>{{ $organizationName }}</strong> to join
            <strong>{{ $allianceName }}</strong>, a Unity Impact Alliance on {{ $appName }}.
        </p>
        ...
        <p style="font-size: 15px; color: #555; margin-bottom: 24px;">
            On that page you can <strong>Accept</strong> or <strong>Decline</strong> the invitation and see your Unity Impact Alliance membership status.
        </p>
